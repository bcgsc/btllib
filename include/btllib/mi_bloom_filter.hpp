...
public:
	...
	*/
	...
	/** 
	* Transform bit vector to interleaved bit vector and create ID array of size of interleaved bit vector.
	* It can be called only once and bit vector insertions cannot be made after calling this function.
	*/
	...
	* hash_num argument used when the Bloom filter was constructed.
	*/
	void insert_bv(const std::vector<uint64_t>& hashes) { insert_bv(hashes.data()); }
	...
	* hash_num argument used when the Bloom filter was constructed.
	*/
	bool bv_contains(const std::vector<uint64_t>& hashes) { return bv_contains(hashes.data()); }
...
template<typename T>
inline MIBloomFilter<T>::MIBloomFilter(size_t bytes, unsigned hash_num, std::string hash_fn):
                        bytes(bytes),
                        hash_num(hash_num),
                        hash_fn(hash_fn){
...
template<typename T>
inline void MIBloomFilter<T>::insert_bv(const uint64_t* hashes){
...
template<typename T>
inline bool MIBloomFilter<T>::bv_contains(const uint64_t* hashes){
        assert(bv_insertion_completed);
        for (unsigned i = 0; i < hash_num; i++){
...
template<typename T>
inline bool MIBloomFilter<T>::complete_bv_insertion(){
...
template<typename T>
inline uint64_t MIBloomFilter<T>::get_pop_cnt() {
        assert(bv_insertion_completed);
