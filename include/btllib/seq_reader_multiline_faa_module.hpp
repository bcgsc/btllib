#ifndef BTLLIB_SEQ_READER_MULTILINE_FAA_MODULE_HPP
#define BTLLIB_SEQ_READER_MULTILINE_FAA_MODULE_HPP
...
/// @cond HIDDEN_SYMBOLS
class SeqReaderMultilineFaaModule
{
                                               ...
                                               RecordType& record)
{
  if (std::ferror(reader.source) == 0 && std::feof(reader.source) == 0) {
    ...
    if (p != EOF) {
      ...
      check_error(ret == EOF, "SeqReaderMultilineFaaModule: ungetc failed.");
      int c;
      for (;;) {
        switch (stage) {
          case Stage::HEADER: {
          ...
          case Stage::TRANSITION: {
            ...
            check_error(ret == EOF,
                        "SeqReaderMultilineFaaModule: ungetc failed.");
            if (c == '>') {
              stage = Stage::HEADER;
              return true;
            }
...
template<typename ReaderType, typename RecordType>
inline bool
SeqReaderMultilineFaaModule::read_file(ReaderType& reader, RecordType& record)
{
