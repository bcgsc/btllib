#ifndef BTLLIB_SEQ_READER_FAA_MODULE_HPP
#define BTLLIB_SEQ_READER_FAA_MODULE_HPP
...
template<typename ReaderType, typename RecordType>
inline bool
SeqReaderFaaModule::read_buffer(ReaderType& reader, RecordType& record)
{
  record.header.clear();
  record.seq.clear();
  record.qual.clear();
...
template<typename ReaderType, typename RecordType>
inline bool
SeqReaderFaaModule::read_transition(ReaderType& reader, RecordType& record)
{
  if (std::ferror(reader.source) == 0 && std::feof(reader.source) == 0) {
    ...
    if (p != EOF) {
      ...
      check_error(ret == EOF, "SeqReaderFaaModule: ungetc failed.");
      switch (stage) {
        case Stage::HEADER: {
...
template<typename ReaderType, typename RecordType>
inline bool
SeqReaderFaaModule::read_file(ReaderType& reader, RecordType& record)
{
